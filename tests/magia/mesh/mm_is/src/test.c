...
uint32_t fsync_cycles[MESH_X_TILES * MESH_Y_TILES * REPETITIONS]; //each tile will save its values in this array (only the cycles spent in synchronization)
...
int main(void){
    ...
    zero_buffer(fsync_cycles, MESH_X_TILES * MESH_Y_TILES * REPETITIONS);
    ...
    uint32_t axi_addr_y = (uint32_t) y_out + (y_id * K_SIZE * tile_h_max * 2);
